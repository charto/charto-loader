...
charto.initSystem = function(config, done, firstConfig) {
	...
	var deps = [
		...
		// 3. Apply SystemJS configuration objects before loading files.
		function() {
			System.config(firstConfig);
			System.config(config);
		},

		// 4. Load external SystemJS configuration file
		// (from address or list of possible addresses).
		configURL && 'config',

		// 5. Patch SystemJS to support Node.js module resolution.
		function() {
			charto.patchSystem(
				System,
...
charto.patchSystem = function(System, done) {
	...
	System.import('cresolve').then(function(pkg) {
...
/** Simple script loader supporting multiple alternative locations,
  ...
  *
  * @param { (string | function)[] } deps - Modules or their names to load
  *   (in order).
